        ...
        function changeLanguage(language) {
            const translations = {
                ...
                "sq": {
                    ...
                    "cities": "Qytetet",
                    "citizens": "Qytetarët",
                    "details": "Ndrysho te dhenat e qytetarit :",
            ...
            if (citizenName) {
                document.getElementById("detailsHeader").textContent = selectedLanguage.details + citizenName.textContent;
            }
...
<main class="container mx-auto px-6 py-8 flex-grow">
    <h1 id="detailsHeader" class="text-4xl font-extrabold text-gray-800 mb-6">Ndrysho te dhenat e qytetarit:<span id="citizenName">{{ $qytetar->emri }} {{ $qytetar->mbiemri }}</span></h1>

    <div class="mb-4 flex items-center justify-between">
        <a href="{{ url('/') }}" id="backToTable" class="text-blue-500 hover:underline text-lg"> &larr; Kthehu tek tabela</a>
    </div>

    <form action="{{ url('/qytetaret/' . $qytetar->id) }}" method="POST" id="updateFrom" class="bg-white shadow-lg rounded-lg p-8">
    @csrf
    @method('PUT')
    ...
    <div class="mb-6">
        ...
        <input type="number" name="viti_i_lindjes" id="viti_i_lindjes" value="{{ $qytetar->viti_i_lindjes }}" required min="1900" max="{{ date('Y') }}"
               class="w-full px-5 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
    </div>
    ...
    <button type="submit" class="bg-gradient-to-r from-blue-500 to-blue-700 text-white px-6 py-3 rounded-full shadow-md transform transition-transform duration-300 hover:scale-105 hover:shadow-xl w-full">
            <span id="updateButtonEdit"> Update </span>
    </button>
</form>
...
<div id="confirmationModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-lg p-6 w-1/3">
        ...
        <div class="flex justify-center gap-4">
            <!-- Here you can add buttons to interact with the modal if needed -->
        </div>
    </div>
</div>
...
<script>
    ...
    const updateForm = document.getElementById('updateFrom');
    ...
    function openModal(emri, mbiemri) {
        ...
        if (language === "en") {
            ...
            modalMessage.textContent = `You have updated the citizen with the name: ${emri} ${mbiemri}.`;
        } else {
            modalTitle.textContent = "Lajmërim!";
            modalMessage.textContent = `Ju keni përditësuar qytetarin me emrin: ${emri} ${mbiemri}.`;
        }
